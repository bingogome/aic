...
def create_dummy_batch(batch_size: int = 4, device: str = "cuda") -> Dict[str, torch.Tensor]:
    """Create a properly shaped batch for testing ACT training."""
    
    # ACT expects images as a list of (B, C, H, W) tensors under "observation.images"
    images = [
        torch.randn(batch_size, 3, 256, 288).to(device),  # left camera
        torch.randn(batch_size, 3, 256, 288).to(device),  # center camera  
        torch.randn(batch_size, 3, 256, 288).to(device),  # right camera
    ]
    
    batch = {
        # Images as list of (B, C, H, W) tensors
...
def test_act_training_loop():
    ...
    for step in range(num_steps):
        ...
        if 'loss' not in output:
            print("❌ No loss in output - this indicates a problem with the training setup")
            return False
    ...
    with torch.no_grad():
        ...
        inference_images = [
            torch.randn(1, 3, 256, 288).to(device),  # left camera
            torch.randn(1, 3, 256, 288).to(device),  # center camera
            torch.randn(1, 3, 256, 288).to(device),  # right camera
        ]
        
        inference_batch = {
            "observation.images": inference_images,
