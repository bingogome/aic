...
def _present(name: str, translation: float, yaw: float = 0.0) -> dict:
    return {
        "entity_present": True,
        "entity_name": name,
        "entity_pose": {"translation": round(translation, 5), "roll": 0.0, "pitch": 0.0, "yaw": round(yaw, 5)},
    }
    ...
    return {
        "scene": {
            ...
            "cables": {
                "cable_0": {
                    "pose": {
                        "gripper_offset": {"x": 0.0, "y": 0.015385, "z": round(cable_z, 5)},
...
def main() -> None:
    ...
    parser.add_argument("--output", type=str, default="/tmp/data_collection_config.yaml",
    ...
    with open(out, "w") as f:
        ...
        f.write(f"# episodes_per_scenario={args.episodes_per_scenario}, seed={args.seed}\n")
        f.write(f"# total_trials={3 * args.episodes_per_scenario}\n\n")
        yaml.dump(config, f, default_flow_style=False, sort_keys=False)

    total = 3 * args.episodes_per_scenario
    print(f"Generated {total} trials ({args.episodes_per_scenario} × 3 scenarios) → {out}")
